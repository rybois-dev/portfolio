...
public class Jeu implements Runnable {
    ...
    public Graphe getGraphe() {
        ...
        for(Tuile tuile : tuiles){
            if(tuile.peutEtreSommet()) map.put(tuile,new Sommet(tuile, this));
        }

        for(Tuile tuile : tuiles){
            if(tuile.peutEtreSommet()){
                Sommet sommet = map.get(tuile);
                for(Tuile tuileVoisine : tuile.getVoisines()){
                    if(tuileVoisine.peutEtreSommet()){
    ...
    public Graphe getGraphe(Joueur joueur) {
        Map<Tuile, Sommet> map = new HashMap<>();
        for(Tuile tuile : tuiles){
            if(tuile.hasRail(joueur)) map.put(tuile,new Sommet(tuile, this));
        }
        for(Tuile tuile : tuiles){
            if(tuile.hasRail(joueur)){
                Sommet sommet = map.get(tuile);
                for(Tuile tuileVoisine : tuile.getVoisines()){
                    if(tuileVoisine.hasRail(joueur)){
                        sommet.ajouterVoisin(map.get(tuileVoisine));
                    }
                }
            }
